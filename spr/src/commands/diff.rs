...
fn synthetic_base_commit_message(updating_pull_request: bool, master_branch: &str) -> String {
    let description = if updating_pull_request {
        "changes introduced through rebase".to_string()
    } else {
        format!("changes to {master_branch} this commit is based on")
    };

    // GitHub may copy this message into the final squash commit.
    format!("[spr] {description}")
}
...
mod tests {
    ...
    #[test]
    fn test_synthetic_base_commit_messages_are_safe_for_squash_merges() {
        assert_eq!(
            synthetic_base_commit_message(false, "main"),
            "[spr] changes to main this commit is based on"
        );
        assert_eq!(
            synthetic_base_commit_message(true, "main"),
